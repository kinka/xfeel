...
### 0.5 智慧干预（Wise Interventions，Walton/Wilson 一系）

**要解决的问题**：理解层让回复“像懂这个人”，但当用户反复用同一种消极方式解释自己
（“我就是不行”“只有我格格不入”），共情回复的默认反应是**跟他讲道理**——
基线 A/B 里模型原话：“但‘永远做不好’这个结论跳得有点快”。这是安慰，也是反驳；
心理学上这恰恰会加固对方的立场。智慧干预的主张是：不由 AI 说出那个积极解释，
而是问一个小问题，让**用户自己说出来**（saying-is-believing）。

模块：`packages/conversation/src/wisdom/`，特性开关 `XFEEL_WISDOM_DISABLED=1` 一键关闭。

1. **认知杠杆识别（Construal Levers）** —— `lever-detector.ts` / `lever-store.ts`
   - 四类：`fixed_attribution`（固定性自我归因）、`belonging_uncertainty`（归属感焦虑）、
     `hostile_attribution`（敌意归因）、`self_continuity_threat`（自我连续性断裂）。
   - **廉价前置门**：先用低落/挫折正则筛，绝大多数消息（记录/提问/好消息）不花 LLM 调用。
     实测真实语料触发率：对话消息 10.2%、历史日记 25.9%。
   - **防幻觉**：命中必须带**逐字**取自原话的 `quote`，否则丢弃——决不能替用户说出他没说过的自我否定。
   - 命中落 `construal_levers` 表（可回溯、可审计），用户侧看不到任何“你被标注了”的痕迹。

2. **干预闸门** —— `wisdom-context.ts`（这个模块一半的价值在于**不干预**）
   - 危机/丧失类内容（自杀念头、亲人离世…）走 `care/guard` 的敏感护栏：**绝不干预**，
     连检测都不跑；那种时刻要的是陪伴，不是一个巧妙的问题。
   - **跨天复现才撬**：同一杠杆按“天”计数（同一晚说三遍只算一天）。单次低落是正常情绪，不是认知模式。
   - **冷却期**默认 3 天：否则每次难过都被反问，会变成骚扰。
   - 阈值均可用环境变量调（`XFEEL_WISDOM_WINDOW_DAYS` / `MIN_DAYS` / `MIN_CONFIDENCE` / `COOLDOWN_DAYS`）。

3. **自作者回收（Saying-is-Believing）** —— `narrative-capture.ts` / `narrative-store.ts`
   - 干预时注入的提示块**禁止**说教、鸡汤、贴标签，并禁止由 AI 说出那个积极解释；
     只允许“先接住情绪 + 问一个锚定在 ta 自己某次成功经历上的小问题”。
   - 只有用户**直接回答**了这一问、并说出自己的解释，才写入 `preferredNarratives`
     （附和/继续自我否定/岔开话题都不算）。AI 生成的漂亮话一律不入库——否则这套机制会退化成鸡汤机。
   - `preferredNarratives` 存在 L3 画像里，但**不是归纳产物**：`profile-builder` 重建（含 full 模式）必须原样保留。

4. **递归证据链** —— `narrative-evidence.ts`（挂在 nightly 归档 + 画像重建之后）
   - 在叙事被说出**之后**的日志里，找真实发生过的、支持这句话的小事，挂到叙事下面。
   - 之后由 `profile-context` 注入回复：引用的是 **ta 自己的话 + ta 自己的事实**，不是我们的鼓励。

#### 0.5.1 A/B 验证结论（`bun run scripts/eval-wisdom.ts`）

合成语料 14 条用例（5 条该干预 / 9 条不该干预，含危机、哀伤、抱怨、求助、好消息等干扰项），
两条分支只差一个开关，跑真实回复路径；回复由 LLM 盲评（不知道来自哪个分支）。

闸门（确定性）：**该干预 5/5 命中；不该干预 0/9 误伤；危机内容开口 0 次。**

回复质量（仅在“该干预”的 5 条上，差异才不会被稀释）：
...
代价：命中干预的那条消息延迟中位数 +2.7s（多一次检测调用 + 一次反例召回）；
未命中的消息（约九成）几乎无额外成本。

结论：**值得引入，但必须默认带着这套闸门**。收益集中在“反复自我否定”这一小类消息上，
而风险（对危机内容耍小聪明、把正常情绪当认知模式反问）全部由闸门挡掉——
闸门比提示词重要得多。
